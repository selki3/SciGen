...
def get_lines(fil):
    ...
    with open(fil, 'r') as f:
        for line in f:
            if line.strip():
                lines.append(word_tokenize(line.strip()))  # Tokenize each line
            else:
                lines.append('empty')
    return lines
...
if __name__ == '__main__':
    ...
    refs = [word_tokenize(sentence) for sentence in get_lines(args.sys)]
    preds = [word_tokenize(sentence) for sentence in get_lines(args.pred)]
